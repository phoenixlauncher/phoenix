  ...
  init() {
    ...
    if logFileURLs != nil && logFileURLs!.count > 3 {
      let sortedLogFileURLs = logFileURLs!.sorted(by: { (url1, url2) -> Bool in
        ...
        let file1ModificationDate = file1Attributes![FileAttributeKey.modificationDate] as! Date
        let file2ModificationDate = file2Attributes![FileAttributeKey.modificationDate] as! Date
